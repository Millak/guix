  ...
  (package
    ...
    (source
     (origin
       ...
       (sha256
        (base32
         "11y9py2sllpj0kv0skb2y43f4cacqaqba3a0bph1pcayyvyvkrg5"))))
    (build-system gnu-build-system)
    (native-inputs
     (list pkg-config))
    (inputs
  ...
  (package
    ...
    (source
     (origin
       ...
       (file-name (git-file-name name version))
       (sha256
        (base32 "04x24izan3g2znnq6lzn3zlq0bl34qad530cp4ynm412w6q97kbz"))))
    (build-system gnu-build-system)
    (arguments
     (list
      #:make-flags
      #~(list (string-append "INSTALL="
                             (search-input-file %build-inputs "/bin/install")))
      #:phases
      #~(modify-phases %standard-phases
            ...
            (lambda* (#:key inputs #:allow-other-keys)
              ...
              (for-each (lambda (file)
                        ...
                        (append (find-files "common" ".*")
                                ...
                                (find-files "tools" ".*")
                                (find-files "src" "\\.(c|sh)$")))))
          (add-before 'bootstrap 'fix-hardcoded-binaries
            (lambda* (#:key make-flags #:allow-other-keys)
...
xfstest's \"~a\" command (with any OPTIONs) as documented below.\n\n"
                ...
                (chmod helper #o755)))))))
    (native-inputs
     (list autoconf-2.71 automake libtool pkg-config))
    (inputs
     (list acl
           attr
           bash-minimal
