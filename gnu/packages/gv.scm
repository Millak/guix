...
(define-public gv
  (package
    ...
    (source
     (origin
       (method url-fetch)
       (uri (string-append "mirror://gnu/gv/gv-" version ".tar.gz"))
       (sha256
        (base32 "0q8s43z14vxm41pfa8s5h9kyyzk1fkwjhkiwbf2x70alm6rv6qi1"))))
              ...
              (let ((gs (search-input-file inputs "bin/gs")))
                (with-fluids ((%default-port-encoding "ISO-8859-1"))
                  (substitute* "src/Makefile.in"
                    (("GV\\.gsInterpreter:([[:blank:]]+)gs" _match
                      blank)
                     (string-append "GV.gsInterpreter:" blank gs))
                    (("GV\\.gsCmd([[:alpha:]]+):([[:blank:]]+)gs"
                      _match command blank)
                     (string-append "GV.gsCmd" command ":" blank gs))))))))))
    (inputs
     (list ghostscript/x
           libx11
           libxaw3d
