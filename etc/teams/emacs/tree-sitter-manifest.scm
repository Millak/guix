...
(define* (is-tree-sitter-package? name)
  (or (string-prefix? "tree-sitter" name)
      (string-prefix? "python-tree-sitter" name)))

(manifest
  (map package->manifest-entry
       (fold-packages
        (lambda (package lst)
          (if (is-tree-sitter-package? (package-name package))
              (cons package lst)
              lst))
        (list))))
