...
(define-module (guix scripts import nuget)
  #:use-module (guix ui)
...
(define (show-help)
  (display (G_ "Usage: guix import nuget PACKAGE-NAME
Import and convert the NuGet package for PACKAGE-NAME.\n"))
  (display (G_ "
...
(define %options
  ...
  (cons* (option '(#\h "help") #f #f
         ...
         (option '(#\V "version") #f #f
                 (lambda args
                   (show-version-and-exit "guix import nuget")))
           ...
           (if (assoc-ref opts 'recursive)
               ...
               (with-error-handling
                 (map package->definition
                      (filter identity
                              (nuget-recursive-import name
                                                     #:version version
                                                     #:repo (or (assoc-ref opts 'repo) 'nuget)
                                                     #:license-prefix prefix-proc))))
               ;; Single import
               (let ((sexp (nuget->guix-package name
                                               #:version version
                                               #:repo (or (assoc-ref opts 'repo) 'nuget)
                                               #:license-prefix prefix-proc)))
                 (unless sexp
